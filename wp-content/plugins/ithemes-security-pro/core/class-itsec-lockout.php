		...
		if ( $itsec_files->get_file_lock( 'lockout_' . $host . $user ) ) {
			...
			if ( $good_host !== false || $good_user !== false ) {
				...
				if ( $good_host !== false && $blacklist_host === false ) { //temp lockout host
					...
					$itsec_logger->log_event( __( 'lockout', 'it-l10n-ithemes-security-pro' ), 10, array( 'expires' => $expiration, 'expires_gmt' => $expiration_gmt, 'type' => $type ), sanitize_text_field( $host ) );
				...
				if ( $good_user !== false ) { //blacklist host and temp lockout user
					...
					if ( $whitelisted === false ) {
						...
						$itsec_logger->log_event( 'lockout', 10, array( __( 'White Listed', 'it-l10n-ithemes-security-pro' ), 'type' => $type ), '', '', intval( $user ) );
					}
	...
	private function send_lockout_email( $host, $user, $host_expiration, $user_expiration, $reason ) {
		...
		if ( $host !== false ) {
			...
			if ( $host_expiration === false ) {
				...
				$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the host lockout you can remove the host from the', 'it-l10n-ithemes-security-pro' ), get_Admin_url( '', 'admin.php?page=toplevel_page_itsec-ban_users' ), __( 'host list', 'it-l10n-ithemes-security-pro' ) );

			} else {

				$host_expiration_text .= sprintf( '<strong>%s %s</strong>', __( 'until', 'it-l10n-ithemes-security-pro' ), sanitize_text_field( $host_expiration ) );
				$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockout please visit', 'it-l10n-ithemes-security-pro' ), get_Admin_url( '', 'admin.php?page=toplevel_page_itsec-ban_users' ), __( 'the admin area', 'it-l10n-ithemes-security-pro' ) );
		...
		if ( $user_object !== false ) {
			...
			if ( $host_text === '' ) {
				...
				$user_text = sprintf( '%s, <strong>%s</strong>, ', __( 'user', 'it-l10n-ithemes-security-pro' ), $user_object->user_login );

				$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockout please visit', 'it-l10n-ithemes-security-pro' ), get_Admin_url( '', 'admin.php?page=toplevel_page_itsec-ban_users' ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );
				...
				$user_text            = sprintf( '%s, <strong>%s</strong>, ', __( 'and a user', 'it-l10n-ithemes-security-pro' ), $user_object->user_login );

				if ( $host_expiration === false ) {

					$release_text .= sprintf( '%s <a href="%s">%s</a>.', __( 'To release the user lockout please visit', 'it-l10n-ithemes-security-pro' ), get_Admin_url( '', 'admin.php?page=toplevel_page_itsec-ban_users' ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );

				} else {

					$release_text = sprintf( '%s <a href="%s">%s</a>.', __( 'To release the lockouts please visit', 'it-l10n-ithemes-security-pro' ), get_Admin_url( '', 'admin.php?page=toplevel_page_itsec-ban_users' ), __( 'the lockouts page', 'it-l10n-ithemes-security-pro' ) );
