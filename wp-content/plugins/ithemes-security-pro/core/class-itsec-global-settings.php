...
class ITSEC_Global_Settings {
	...
	public function email_notifications() {
		...
		printf( '<p class="description">%s<a href="admin.php?page=toplevel_page_itsec_settings">%s</a>%s</p>', __( 'This feature will trigger an email to be sent to the ', 'it-l10n-ithemes-security-pro' ), __( 'notifications email address', 'it-l10n-ithemes-security-pro' ), __( ' whenever a host or user is locked out of the system.', 'it-l10n-ithemes-security-pro' ) );
	...
	public function register_tooltip( $tooltip_modules ) {
		...
		if ( ! isset ( $this->settings['allow_tracking'] ) || $this->settings['allow_tracking'] === false ) {
			...
			$tooltip_modules['tracking'] = array(
				...
				'text'      => sprintf( '%s %s', $itsec_globals['plugin_name'], __( 'would like to collect anonymous data about features you use to help improve this plugin. Absolutely no information that can identify you will be collected.', 'it-l10n-ithemes-security-pro' ) ),
	...
	public function sanitize_module_input( $input ) {
		...
		if ( isset( $input['notification_email'] ) ) {
			...
			if ( sizeof( $bad_emails ) > 0 ) {
				...
				$bad_addresses = implode( ', ', $bad_emails );
				$type          = 'error';
				$message       = __( 'The following notification email address(es) do not appear to be valid: ', 'it-l10n-ithemes-security-pro' ) . $bad_addresses;
		...
		if ( sizeof( $bad_white_listed_ips ) > 0 ) {
			...
			$message .= sprintf( '%s<br /><br />', __( 'There is a problem with an IP address in the white list:', 'it-l10n-ithemes-security-pro' ) );

			foreach ( $bad_white_listed_ips as $bad_ip ) {
				$message .= sprintf( '%s %s<br />', $bad_ip, __( 'is not a valid address in the white list users box.', 'it-l10n-ithemes-security-pro' ) );
			}
	...
	public function write_files() {
		...
		printf(
			...
			$server_file,
			__( ' automatically. If disabled you will need to manually place configuration options in those files.', 'it-l10n-ithemes-security-pro' )
		);
