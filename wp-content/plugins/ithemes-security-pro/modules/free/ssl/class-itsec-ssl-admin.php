...
class ITSEC_SSL_Admin {
	...
	public function dashboard_status( $statuses ) {
		...
		if ( FORCE_SSL_LOGIN === true && FORCE_SSL_ADMIN === true ) {
			...
			$status       = array( 'text' => __( 'You are requiring a secure connection for logins and the admin area.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_ssl_login', );

		} elseif ( FORCE_SSL_LOGIN === true || FORCE_SSL_ADMIN === true ) {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You are requiring a secure connection for logins or the admin area but not both.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_ssl_login', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You are not requiring a secure connection for logins or for the admin area.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_ssl_login', );
