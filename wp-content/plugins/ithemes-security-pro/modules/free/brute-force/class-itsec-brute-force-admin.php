...
class ITSEC_Brute_Force_Admin {
	...
	public function brute_force_check_period() {
		...
		$content .= '<label for="itsec_brute_force_check_period"> ' . __( 'Minutes', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description"> ' . __( 'The number of minutes in which bad logins should be remembered.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function brute_force_enabled() {
		...
		$content .= '<label for="itsec_brute_force_enabled"> ' . __( 'Enable brute force protection.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function brute_force_max_attempts_host() {
		...
		$content .= '<label for="itsec_brute_force_max_attempts_host"> ' . __( 'Attempts', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description"> ' . __( 'The number of login attempts a user has before their host or computer is locked out of the system. Set to 0 to record bad login attempts without locking out the host.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function brute_force_max_attempts_user() {
		...
		$content .= '<label for="itsec_brute_force_max_attempts_user"> ' . __( 'Attempts', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'Your login area is protected from brute force attacks.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_brute_force_enabled', );

		} else {

			$status_array = 'high';
			$status       = array( 'text' => __( 'Your login area is not protected from brute force attacks.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_brute_force_enabled', );
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Brute Force Protection', 'it-l10n-ithemes-security-pro' ),
	...
	public function logs_metabox() {
		...
		echo __( 'Below is the log of all the invalid login attempts in the WordPress Database. To adjust logging options visit the global settings page.', 'it-l10n-ithemes-security-pro' );
