...
class ITSEC_Hide_Backend_Admin {
	...
	public static function build_rewrite_rules( $input = null ) {
		...
		if ( $input['enabled'] == true ) {
			...
			if ( $server_type == 'nginx' ) {
				...
				$rules .= "\t# " . __( 'Rules to hide the dashboard', 'it-l10n-ithemes-security-pro' ) . PHP_EOL . "\trewrite ^" . $home_root . $input['slug'] . "/?$ /wp-login.php?\$query_string break;" . PHP_EOL;

			} else {

				$rules .= "\t# " . __( 'Rules to hide the dashboard', 'it-l10n-ithemes-security-pro' ) . PHP_EOL . "\tRewriteRule ^" . $home_root . $input['slug'] . "/?$ /wp-login.php [QSA,L]" . PHP_EOL;
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'Your WordPress Dashboard is hidden.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_hide_backend_enabled', );

		} else {

			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your WordPress Dashboard is using the default addresses. This can make a brute force attack much easier.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_hide_backend_enabled', );
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Custom Login Slug', 'it-l10n-ithemes-security-pro' ),
	...
	public function sanitize_module_input( $input ) {
		...
		if ( in_array( trim( $input['slug'] ), $forbidden_slugs ) && $input['enabled'] === true ) {
			...
			$type    = 'error';
			$message = __( 'Invalid hide login slug used. The login url slug cannot be "login," "admin," "dashboard," or "wp-login.php" ob "" (blank) as these are use by default in WordPress.', 'it-l10n-ithemes-security-pro' );
	...
	private function send_new_slug( $new_slug ) {
		...
		$subject    = '[' . get_option( 'siteurl' ) . '] ' . __( 'WordPress Login Email Changed', 'it-l10n-ithemes-security-pro' );
