...
class ITSEC_Ban_Users_Admin {
	...
	public function ban_users_host_list() {
		...
		$content .= '<p>' . __( 'Use the guidelines below to enter hosts that will not be allowed access to your site. Note you cannot ban yourself.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		$content .= '<ul>';
		$content .= '<li>' . __( 'You may ban users by individual IP address or IP address range.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		$content .= '<li>' . __( 'Individual IP addesses must be in IPV4 standard format (i.e. ###.###.###.### or ###.###.###.###/##). Wildcards (*) or a netmask is allowed to specify a range of ip addresses.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		$content .= '<li>' . __( 'If using a wildcard (*) you must start with the right-most number in the ip field. For example ###.###.###.* and ###.###.*.* are permitted but ###.###.*.### is not.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		$content .= '<li><a href="http://ip-lookup.net/domain-lookup.php" target="_blank">' . __( 'Lookup IP Address.', 'it-l10n-ithemes-security-pro' ) . '</a></li>';
		$content .= '<li>' . __( 'Enter only 1 IP address or 1 IP address range per line.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		$content .= '</ul>';
	...
	public function ban_users_white_list() {
		...
		echo '<p>' . __( 'Use the guidelines below to enter hosts that will not be banned from your site. This will keep you from locking yourself out of any features if you should trigger a lockout. Please note this does not override away mode.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<ul>';
		...
		echo '</ul>';
		echo '<p class="description"><strong>' . __( 'Note: this white list is only to prevent your IP from getting banned at the server level (which is much more difficult to clear). If you want to whitelist your IP from all lockouts use the whitelist option in the Global Settings.', 'it-l10n-ithemes-security-pro' ) . '</strong></p>';
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array( 'text' => __( 'You are blocking known bad hosts and agents with the ban users tool.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_ban_users_enabled', );

		} else {

			$status_array = 'low';
			$status       = array( 'text' => __( 'You are not blocking any users that are known to be a problem. Consider turning on the Ban Users feature.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_ban_users_enabled', );
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Whitelist Users', 'it-l10n-ithemes-security-pro' ),
	...
	public function sanitize_module_input( $input ) {
		...
		if ( sizeof( $bad_white_listed_ips ) > 0 ) {
			...
			foreach ( $bad_white_listed_ips as $bad_ip ) {
				$message .= sprintf( '%s %s<br />', $bad_ip, __( 'is not a valid address in the whitelist users box.', 'it-l10n-ithemes-security-pro' ) );
			}
