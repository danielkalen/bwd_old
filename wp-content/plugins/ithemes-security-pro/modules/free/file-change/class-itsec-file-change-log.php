...
final class ITSEC_File_Change_Log extends ITSEC_WP_List_Table {
	...
	function column_detail( $item ) {
		...
		if ( $item['added'] > 0 || $item['removed'] > 0 || $item['changed'] > 0 ) {
			...
			$content .= '<h3>' . __( 'Files Added', 'it-l10n-ithemes-security-pro' ) . '</h3>';

			$content .= '<ol class="file_change_detail_list">';

			foreach ( $item['added_detail'] as $file => $details ) {
				$content .= '<li class="file_change_detail"><strong>' . __( 'File', 'it-l10n-ithemes-security-pro' ) . '</strong>: ' . $file . '<br /><strong>' . __( 'Date', 'it-l10n-ithemes-security-pro' ) . '</strong>: ' . date( ' Y-m-d g:i a', $details['mod_date'] ) . '</li>';
			}

			$content .= '</ol>';

			$content .= '<h3>' . __( 'Files Removed', 'it-l10n-ithemes-security-pro' ) . '</h3>';
			...
			$content .= '<h3>' . __( 'Files Changed', 'it-l10n-ithemes-security-pro' ) . '</h3>';
