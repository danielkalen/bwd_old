...
$intense_post_type = $intense_post_types->get_post_type( 'intense_news' );
	...
	<div class='intense row' style='<?php echo $intense_custom_post['cancel_plugin_layout_style']; ?>'>
		<div class='intense col-lg-12 col-md-12 col-sm-12 col-xs-12'>
			<div class='post-header'>
				...
				<?php
					if ( get_field( 'intense_news_subtitle' ) != '' ) {
						echo '<h3>' . get_field( 'intense_news_subtitle' ) . '</h3>';
					}

					$author = '';
					$publisher = '';

					if ( get_field( 'intense_news_author' ) != '' ) {
						$author = get_field( 'intense_news_author' );
					}

					if ( get_field( 'intense_news_publisher' ) != '' ) {
						$publisher = get_field( 'intense_news_publisher' );

						if ( get_field( 'intense_news_publisher_website' ) != '' ) {
							$publisher = '<a href="' . get_field( 'intense_news_publisher_website' ) . '" target="_blank">' . $publisher . '</a>';
						}
					}

					if ( $author != '' ) {
						echo __('by', 'intense' ) . ': ' . $author . ', ' . $publisher;
					}

					if ( get_field( 'intense_news_author' ) != '' ) {
						$author = get_field( 'intense_news_author' );
						echo "    " . __('View original article ', 'intense' ) . '<a href="' . get_field( 'intense_news_website' ) . '" target="_blank">here</a>';
					}
				?>
				
				<div class='entry-content' style='text-decoration:italic; font-size:small;'>
					<?php echo __('Date', 'intense' ) ?>: <?php echo date("M d, Y", strtotime( get_field( 'intense_news_date' ) ) ); ?>
				</div>
			</div>	
		</div>
	</div>
	...
	<div class='intense row' style='<?php echo $intense_custom_post['cancel_plugin_layout_style']; ?>'>
		<div class='intense col-lg-12 col-md-12 col-sm-12 col-xs-12' style='position: relative;'>
			<?php echo intense_get_post_thumbnails( 
				( isset( $intense_custom_post['image_size'] ) ? $intense_custom_post['image_size'] : 'postExtraWide' ), 
				null, 
				false, 
