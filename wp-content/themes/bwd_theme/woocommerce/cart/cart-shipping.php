...
// $cart_total = print_r(WC()->cart);
?>
<tr class="cart-totals-table-row shipping">
	<th class="cart-totals-table-row-heading"><?php
	...
	?></th>
	<td class="cart-totals-table-row-value">

		<?php //if ( $cart_total > 120 ) { // Free Shipping threshhold ?>
			<?php if ( ! empty( $available_methods ) ) : ?>
				...
				<?php elseif ( get_option( 'woocommerce_shipping_method_format' ) === 'select' ) : ?>
					...
					<select class="cart-totals-table-row-value-select shipping_method" name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>" class="shipping_method">
			...
			<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
				...
				<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
					...
					<p><?php _e( 'Not Calculated', 'woocommerce' ); ?></p>
