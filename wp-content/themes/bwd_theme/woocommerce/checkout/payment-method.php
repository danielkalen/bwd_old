...
'<div class="checkout-payment-stripe payment_method_' . $gateway->id. '">';
	...
	?>
	<input id='payment_method_<?php echo $gateway->id ?>' type='radio' class='checkout-payment-stripe-radio input-radio' name='payment_method' value='<?php echo esc_attr( $gateway->id ) ?>' <?php checked( $gateway->chosen, true ) ?> data-order_button_text='<?php echo esc_attr( $gateway->order_button_text ) ?>' />
